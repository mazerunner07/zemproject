...
export default function PaymentsPage({ userProjects }: { userProjects: DetailedUserProjects }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-12 gap-6 p-4 md:p-6 bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100">
      {/* Projects List */}
        ...
        <ScrollArea className="h-[500px] w-full rounded-md p-2">
          {userProjects.map((project) => (
            <Link key={project.id} href={`/dashboard/payments?pId=${project.id}`}>
              <Card
                className={cn(
                  "hover:bg-slate-100 dark:hover:bg-gray-700 cursor-pointer p-4 flex items-center gap-4 hover:shadow-lg transition rounded-lg mb-2",
                  project.id === selectedProjectId ? "bg-slate-300 dark:bg-gray-600" : "bg-white dark:bg-gray-800"
                )}
              >
                <Image
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {selectedProject.payments?.map((payment) => (
                      <tr key={payment.id} className="border-b hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        <td className="py-3 px-4">
                          <Button size="sm" asChild className="dark:bg-gray-700 dark:text-gray-100">
