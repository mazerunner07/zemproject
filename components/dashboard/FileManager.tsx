...
import { MoreVertical, Folder, Calendar, HardDrive, Clock, AlignJustify, Menu, X } from "lucide-react"
import { FaFilePdf, FaImage, FaFileWord, FaFileExcel, FaFileArchive, FaFilePowerpoint, FaFileAlt } from "react-icons/fa"
import { cn } from "@/lib/utils"
import { Button } from "@/components/ui/button"
import { ScrollArea } from "@/components/ui/scroll-area"
import { Sheet, SheetContent, SheetHeader, SheetTitle } from "@/components/ui/sheet"
import FolderForm from "../Forms/FolderForm"
import type { UserFolder } from "@/types/types"
...
export default function FileManager({ userId, userFolders }: { userId: string; userFolders: UserFolder[] }) {
  ...
  // Instead of returning early, we'll handle the no-folder case within the component
  const folderExists = !!selectedFolder
  const totalSpace = folderExists ? Number(((2 * 1073741824) / userFolders.length).toFixed(2)) : 2 * 1073741824 // Default to 2GB if no folder
  const usedSpace = folderExists ? selectedFolder.files.reduce((acc, item) => acc + item.size, 0) : 0
  const usagePercentage = totalSpace > 0 ? (usedSpace * 100) / totalSpace : 0
  ...
  function getFileIcon(mimeType: string | undefined) {
    ...
    const iconMap: Record<string, JSX.Element> = {
      ...
      "application/vnd.openxmlformats-officedocument.presentationml.presentation": (
        <FaFilePowerpoint className="w-6 h-6 text-orange-500" />
      ),
        ...
        <ScrollArea className="h-[calc(100vh-8rem)]">
          <div className="space-y-1">
            {userFolders.length === 0 ? (
              <p className="text-sm text-center text-muted-foreground p-4">No folders yet</p>
            ) : (
      ...
      <div className="flex-1 flex flex-col overflow-hidden dark:bg-[#0F172A]">
        ...
        <div className="p-4 overflow-y-auto">
          {!folderExists ? (
            ...
            <>
              <div
                className="relative bg-yellow-300 dark:bg-[#1E293B] p-5 mb-5 w-full"
                style={{
                  clipPath: "polygon(51% 0, 66% 33%, 100% 34%, 100% 100%, 0 100%, 0% 60%, 0 0)",
                  borderRadius: "2rem",
                }}
              >
                {/* Content */}
                <div className="relative z-10">
                  ...
                  <p className="text-sm text-gray-600 mt-1 dark:text-white/50">
                    📅 {getNormalDate(selectedFolder.createdAt)}
                  </p>
                  <p className="text-sm text-gray-700 mt-1 dark:text-white/50">
                    ...
                    <div className="h-2 w-full bg-secondary dark:bg-white rounded-full overflow-hidden">
                      <div
                        className={cn(
                          ...
                          usagePercentage > 90 ? "bg-red-500" : usagePercentage > 50 ? "bg-lime-500" : "bg-green-500",
                        )}
